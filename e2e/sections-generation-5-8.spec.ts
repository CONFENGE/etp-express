/**
 * E2E Tests - Section Generation (Sections 5-8) - Lei 14.133/2021
 *
 ...
 * - Seção V: Levantamento de Mercado
 * - Seção VI: Estimativa de Preços
 * - Seção VII: Justificativa para o Parcelamento ou não da Solução
 * - Seção VIII: Adequação Orçamentária
 *
 * @requirements
...
/**
 * Section definitions for tests (5-8) - Lei 14.133/2021
...
function createMockSectionResponse(sectionNumber: number) {
  const sectionContents: Record<number, string> = {
    5: 'Levantamento de Mercado: Foram identificados 5 fornecedores potenciais no mercado. Análise de soluções similares em outros órgãos públicos. Verificação de contratos vigentes no Painel de Preços. Consulta ao PNCP para licitações anteriores.',
    6: 'Estimativa de Preços: Valor estimado R$ 150.000,00 (cento e cinquenta mil reais). Metodologia: pesquisa de mercado com 3 fornecedores, consulta ao Painel de Preços, análise de contratações similares.',
    7: 'Justificativa para não parcelamento: A solução deve ser contratada de forma integral para garantir a compatibilidade técnica entre os módulos e evitar riscos de integração. O parcelamento não é economicamente vantajoso conforme análise de mercado.',
    8: 'Adequação Orçamentária: Disponibilidade confirmada na dotação orçamentária 2024. Programa de Trabalho: 12345678. Natureza da Despesa: 44.90.52. Fonte de Recursos: Tesouro.',
  };
...
/**
 * Main test suite for Section Generation (5-8) - Lei 14.133/2021
 */
test.describe('Section Generation - Sections 5-8 (Lei 14.133/2021)', () => {
...
/**
 * Test suite for section-specific content validation (5-8) - Lei 14.133/2021
 */
test.describe('Section Content Validation - Sections 5-8 (Lei 14.133/2021)', () => {
  ...
  /**
   * Test Levantamento de Mercado (Section 5) specific content
   */
  test('should generate valid Levantamento de Mercado content', async ({
    page,
  }) => {
    await page.route('**/api/sections/etp/*/generate', (route) => {
      route.fulfill({
  ...
  /**
   * Test Justificativa Parcelamento (Section 7) with different scenarios
   */
  test('should handle parcelamento variations', async ({ page }) => {
    const parcelamentoResponse = {
      data: {
        id: 'mock-parcelamento-section',
        sectionNumber: 7,
        content:
          'Justificativa para não parcelamento: A solução deve ser contratada integralmente para garantir compatibilidade técnica.',
        status: 'completed',
        metadata: {
          parcelamento: false,
          justificativa: 'compatibilidade_tecnica',
        },
      },
      disclaimer: 'Análise de parcelamento conforme Lei 14.133/2021.',
    };
    ...
    await page.route('**/api/sections/etp/*/generate', (route) => {
      route.fulfill({
        status: 201,
        contentType: 'application/json',
        body: JSON.stringify(parcelamentoResponse),
      });
    });
  ...
  /**
   * Test Adequação Orçamentária (Section 8) with different budget sources
   */
  test('should handle adequacao orcamentaria variations', async ({ page }) => {
    const fontes = ['tesouro', 'convenio', 'proprio'];

    for (const fonte of fontes) {
      const adequacaoResponse = {
        data: {
          id: `mock-adequacao-${fonte}`,
          sectionNumber: 8,
          content: `Adequação Orçamentária: Fonte de recursos ${fonte.toUpperCase()}. Disponibilidade confirmada na dotação orçamentária.`,
          status: 'completed',
          metadata: {
            fonteRecursos: fonte,
          },
        },
        disclaimer: 'Adequação orçamentária conforme Lei 14.133/2021.',
      };
      ...
      await page.route('**/api/sections/etp/*/generate', (route) => {
        route.fulfill({
          status: 201,
          contentType: 'application/json',
          body: JSON.stringify(adequacaoResponse),
        });
      });
