...
export class EtpsService {
  ...
  async findOne(id: string, userId?: string): Promise<Etp> {
    ...
    if (userId && etp.createdById !== userId) {
      ...
      this.logger.warn(`User ${userId} accessed ETP ${id} owned by ${etp.createdById}`);
    }

    return etp;
  }

  async update(id: string, updateEtpDto: UpdateEtpDto, userId: string): Promise<Etp> {
    const etp = await this.findOne(id);

    // Check ownership
    if (etp.createdById !== userId) {
      throw new ForbiddenException('Você não tem permissão para editar este ETP');
    }
  ...
  async updateStatus(id: string, status: EtpStatus, userId: string): Promise<Etp> {
    const etp = await this.findOne(id);

    if (etp.createdById !== userId) {
      throw new ForbiddenException('Você não tem permissão para alterar o status deste ETP');
    }
  ...
  async updateCompletionPercentage(id: string): Promise<void> {
    ...
    if (totalSections === 0) {
      ...
      const completedSections = etp.sections.filter(
        (s) => s.status === 'generated' || s.status === 'reviewed' || s.status === 'approved',
      ).length;
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (etp.createdById !== userId) {
      throw new ForbiddenException('Você não tem permissão para deletar este ETP');
    }
  ...
  async getStatistics(userId?: string) {
    ...
    return {
      ...
      averageCompletion: parseFloat(avgCompletion?.avgCompletion || '0').toFixed(2),
    };
  }
}
