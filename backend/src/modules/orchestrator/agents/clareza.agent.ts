...
export class ClarezaAgent {
  ...
  async analyze(content: string): Promise<ClarezaResult> {
    ...
    if (avgSentenceLength > 25) {
      ...
      suggestions.push('Divida frases longas em frases mais curtas para melhor compreensão');
    }
    ...
    if (jargonCount > words.length * 0.1) {
      ...
      suggestions.push('Considere explicar termos técnicos ou usar linguagem mais acessível');
    }

    // Check for passive voice overuse
    if (passiveVoice > sentences.length * 0.3) {
      issues.push('Uso excessivo de voz passiva');
      suggestions.push('Prefira voz ativa para tornar o texto mais direto e claro');
    }
    ...
    if (complexWordPercentage > 20) {
      ...
      suggestions.push('Simplifique vocabulário quando possível sem perder precisão');
    }
    ...
    const readabilityIndex = this.calculateReadability(avgSentenceLength, avgWordLength);

    // Calculate overall score
    const score = Math.max(0, Math.min(100, readabilityIndex));

    this.logger.log(`Clarity analysis completed. Score: ${score}%, Readability: ${readabilityIndex}`);
