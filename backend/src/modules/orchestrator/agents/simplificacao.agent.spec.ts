...
describe('SimplificacaoAgent', () => {
  ...
  describe('analyze()', () => {
    ...
    it('should detect excessive use of "que"', async () => {
      ...
      expect(result.simplifiedSuggestions).toEqual(
        expect.arrayContaining([expect.stringMatching(/uso excessivo.*"que"/i)]),
      );
    });

    describe('Edge cases', () => {
      it('should handle text with no issues', async () => {
        const perfectText = 'Texto perfeito sem qualquer problema de simplicidade';
  ...
  describe('simplify()', () => {
    it('should apply automatic simplifications', async () => {
      const text = 'Tendo em vista que faz-se necessário planejar antecipadamente';
