...
export class ExportService {
  ...
  private loadTemplate() {
    try {
      const templatePath = path.join(__dirname, 'templates', 'etp-template.hbs');
      ...
      this.template = Handlebars.compile('<html><body><h1>{{etp.title}}</h1></body></html>');
    }
  }
  ...
  async exportToXML(etpId: string): Promise<string> {
    ...
    xml += '  <disclaimer>O ETP Express pode cometer erros. Lembre-se de verificar todas as informações antes de realizar qualquer encaminhamento.</disclaimer>\n';
    xml += '</etp>\n';
