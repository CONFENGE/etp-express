...
- [ ] `TermoReferencia.versao` depreciado e removido (ou marcado `@deprecated`)
- [ ] `TermoReferencia.currentVersion` e a unica fonte de verdade para versionamento
- [ ] Migration criada para unificar dados de `versao` em `currentVersion`
- [ ] `Etp.createdById` declarado com `type: 'uuid'` no decorator `@Column`
- [ ] Migration criada para alterar tipo da coluna `created_by` para `uuid`
- [ ] `ContratoSyncLog.resolution` tipado como `ContratoResolution | null` (nao `any`)
- [ ] `ConflictField.localValue` e `.remoteValue` tipados (nao `any`)
- [ ] Interfaces `ContratoResolution` e `ConflictFieldValue` criadas e exportadas
- [ ] Zero erros de TypeScript apos as mudancas
- [ ] Todos os testes existentes passam apos as alteracoes
