...
- [x] `Medicao` possui coluna `organizationId` (uuid, nullable) com `@ManyToOne` para Organization
- [x] `Ocorrencia` possui coluna `organizationId` (uuid, nullable) com `@ManyToOne` para Organization
- [x] Migration de backfill popula `organizationId` em Medicao via JOIN com Contrato
- [x] Migration de backfill popula `organizationId` em Ocorrencia via JOIN com Contrato
- [x] `Contrato` possui `@OneToMany(() => Medicao, ...)` sem `eager: true`
- [x] `Contrato` possui `@OneToMany(() => Ocorrencia, ...)` sem `eager: true`
- [x] Index GIN criado em `etps.metadata` com `jsonb_path_ops`
- [x] Index GIN criado em `etps.dynamicFields` com `jsonb_path_ops`
- [x] Index GIN criado em `contract_prices.metadata` com `jsonb_path_ops`
- [x] Migrations usam `CREATE INDEX CONCURRENTLY` (sem transacao)
- [x] TenantGuard funciona para queries diretas em Medicao/Ocorrencia
- [x] Todos os testes passam apos as alteracoes
